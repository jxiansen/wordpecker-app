...
export const apiService = {
  ...
  createList: (data: Partial<WordList>): ApiResponse<WordList> =>
    api.post("/api/lists", data),
  updateList: (id: string, data: Partial<WordList>): ApiResponse<WordList> =>
    api.put(`/api/lists/${id}`, data),
  deleteList: async (listId: string): Promise<void> => {
    await api.delete(`/api/lists/${listId}`);
  },

  // Words
  getWords: (listId: string): ApiResponse<Word[]> =>
    api.get(`/api/lists/${listId}/words`),
  addWord: (listId: string, word: string): ApiResponse<Word> =>
    api.post(`/api/lists/${listId}/words`, { word }),
  deleteWord: (listId: string, wordId: string): ApiResponse<void> =>
    api.delete(`/api/lists/${listId}/words/${wordId}`),
